...
accumulate_hydro_p( hydro_array_t              * RESTRICT ha,
  ...
  hydro_t         * RESTRICT ALIGNED(128) d_h;
  particle_t      * RESTRICT ALIGNED(128) d_p;
  interpolator_t  * RESTRICT ALIGNED(128) d_f;

  cudaMalloc((void **)&d_h, sizeof(hydro_t) * ha->g->nv);
  cudaMalloc((void **)&d_p, sizeof(particle_t) * np); 
  cudaMalloc((void **)&d_f, sizeof(interpolator_t) * ia->g->nv);

  cudaMemcpy(d_h, h, sizeof(hydro_t) * ha->g->nv, cudaMemcpyHostToDevice);
  cudaMemcpy(d_p, p, sizeof(particle_t) * np, cudaMemcpyHostToDevice);
  cudaMemcpy(d_f, f, sizeof(interpolator_t) * ia->g->nv, cudaMemcpyHostToDevice);
  // np, qdt_2mc, r8V, qsp, mspc, stride_10, stride_21, stride_43 are to be passed into device codes

  // try to fully occupy all the SMs
  cuda_accumulate_hydro_p<<<num_sm*num_blk_per_sm, num_thread_per_blk>>>(d_h,
                                                                         d_p,
                                                                         d_f,
                                                                         np,
                                                                         c,
  ...
  cudaMemcpy(h, d_h, sizeof(hydro_t) * ha->g->nv, cudaMemcpyDeviceToHost);

  cudaFree(d_h); cudaFree(d_p); cudaFree(d_f);
}
